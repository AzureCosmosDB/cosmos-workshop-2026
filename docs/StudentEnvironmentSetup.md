...
az login
```
 use the `UserPrincipalName` and the same shared password used for the VM. This avoids an
...
|---------|-------------------|-----|
| `No active Azure CLI session` | Azure CLI is not signed in | Run `az login`, then complete sign-in as prompted |
